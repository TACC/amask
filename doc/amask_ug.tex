...
Figure \ref{fig:lstopo} shows an \verb+lstopo+ report for this system (Sandy Bridge compute 
...
machine's processor-ids (\verb+Core P#+ in \ref{fig:lstopo1}) from 0 to Nprocs-1, where Nprocs is 
...
representing processors 0 - 15.
Numbers in a row position mean that the mask bit is set for the the corresponding
processor (for the MPI rank, process, listed at the beginning of the row can 
